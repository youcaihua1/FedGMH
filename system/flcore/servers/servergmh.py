...
from flcore.clients.clientgmh import ClientGMH
...
def read_client_data(dataset, idx, is_train=True):
    if is_train:
        ...
        y_train_np = y_train.numpy()  # me
        # 使用 numpy.bincount 来统计元素出现的次数
        counts = np.bincount(y_train_np, minlength=y_train_np.max() + 1)  # me
        weight = counts / float(len(y_train_np))  # me
        label_weight = {i: weight[i] for i in range(len(weight))}  # me
        label_weight = {k: v for k, v in label_weight.items() if v != 0}  # me
...
class FedGMH(Server):
    ...
    def __init__(self, args, times):
        ...
        self.set_clients(ClientGMH)
        print(f"\ntau: {self.clients[0].args.tau}")
        ...
        self.heads = defaultdict()  # 全局多头部，每一个全局头部处理一种标签信息。
